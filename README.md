...
1. **بازه‌های IP** تعریف‌شده شما (به‌صورت CIDR) را برای دو نوع دستگاه اسکن می‌کند:
   - **سرورهای HPE ProLiant** — از طریق API سِ Redfish روی iLO/BMC.
   - **آرایه‌های ذخیره‌سازی HPE MSA** — از طریق XML API مخصوص MSA.
2. برای هر سرور/ذخیره‌سازی کشف‌شده، یک **device** در NetBox **می‌سازد یا به‌روز می‌کند**؛ شامل سازنده، نوع دستگاه، نقش، سایت، سریال‌نامبر و فیلدهای سفارشی (IP بورد BMC، نسخه فریم‌ور، خلاصه CPU/RAM/دیسک، وضعیت سلامت و …).
3. **انventory سخت‌افزاری دقیق** هر دستگاه (CPU، ماژول‌های RAM، دیسک‌ها، پاورها، کارت‌های شبکه، HBA، کنترلرها، باتری‌ها، FRU) را جمع‌آوری و هر قطعه را به‌عنوان یک **inventory item** در NetBox با کلید سریال‌نامبر همگام می‌کند.
4. **آیتم‌های inventory قدیمی** که دیگر توسط دستگاه گزارش نمی‌شوند را حذف می‌کند.
5. **دستگاه‌هایی که دیگر پاسخ نمی‌دهند** را در NetBox به‌صورت آفلاین (offline) علامت‌گذاری می‌کند.
6. **به‌صورت خودکار و طبق زمان‌بندی** اجرا می‌شود (پیش‌فرض: هر روز ساعت ۰۰:۰۰ و ۱۲:۰۰) به‌علاوه یک اجرای بلافاصله هنگام راه‌اندازی.
...
- **Idempotent (تکرارپذیر)**: هر اجرا وضعیت NetBox را با واقعیت تطبیق می‌دهد. اجرای مجدد بی‌خطر است.
- **اسکن موازی**: probing IP با thread pool انجام می‌شود (`SCAN_WORKERS`، پیش‌فرض ۲۰).
- **مقاوم (Resilient)**: هر دستگاه در `try/except` جداگانه است — خرابی یک میزبان کل اجرا را قطع نمی‌کند. Rate-limit مخصوص MSA با backoff نمایی و ورود مجدد مدیریت می‌شود.
- **انventory با کلید سریال**: قطعات با سریال‌نامبر تطبیق/به‌روزرسانی می‌شوند؛ قطعات قدیمی حذف، تکراری‌ها پاکسازی می‌شوند.
- **بدون secret واقعی در مخزن**: تمام اعتبارها، IPها و نگاشت سایت‌ها در `.env` (که gitignore شده) قرار دارند. فقط `.env.example` در مخزن است.

## فایل‌های مخزن

| فایل | کاربرد |
|------|--------|
| `sync_all_to_netbox.py` | اسکریپت اصلی اتوماسیون — اسکنر، collectorها، همگام‌سازی NetBox و زمان‌بند. |
| `models.py` | نگاشت‌های نرمال‌سازی نام مدل سرور (`SERVER_MODEL_MAP`) و ذخیره‌سازی (`STORAGE_MODEL_MAP`). رشته‌های سازنده (مثلاً `proliant dl360 gen10`) را به نام‌های متعارف نوع دستگاه در NetBox (مثلاً `HPE DL360 G10`) تبدیل می‌کند. |
| `.env.example` | قالب فایل `.env` شما. آن را به `.env` کپی کرده و مقادیر واقعی را وارد کنید. |
| `requirements` | وابستگی‌های پایتون (`requests`, `pynetbox`, `schedule`, `python-dotenv`). |
| `.gitignore` | `.env`، `__pycache__/`، venv و پوشه‌های کاری شخصی را نادیده می‌گیرد. |

## پیش‌نیازها

- پایتون ۳.۸ به بالا
- یک نمونه **NetBox** (نسخه ۳.x) در دسترس با token سِ API
- دسترسی شبکه از ماشینی که این اسکریپت روی آن اجرا می‌شود به:
  - IPهای iLO/BMC روی `REDFISH_PORT` (پیش‌فرض ۴۴۳)
  - IPهای ذخیره‌سازی MSA روی `STORAGE_PORT` (پیش‌فرض ۴۴۳)
- سرورهای HPE ProLiant با iLO 4 / iLO 5 (پشتیبان Redfish)
- آرایه‌های ذخیره‌سازی HPE MSA (نسل ۲۰۴۰ / ۲۰۴۲ / ۲۰۵۰ / ۲۰۵۲ / ۲۰۶۰)
...
فایل `.env.example` را به `.env` کپی کرده و ویرایش کنید. **تمام مقادیر حساس باید در `.env` باشند** — این فایل هرگز به git push نمی‌شود.

| متغیر | الزامی | پیش‌فرض | توضیح |
|--------|:------:|---------|-------|
| `NETBOX_URL` | ✅ | — | آدرس پایه NetBox شما (مثلاً `https://netbox.example.com`). |
...
| `STORAGE_PORT` | ❌ | `443` | پورت TCP سِ XML API مخصوص MSA. |
| `STORAGE_AUTH_HASH` | ❌ | `sha256` | الگوریتم hash برای اعتبار MSA (`sha256` یا `md5`). در صورت شکست، به‌طور خودکار جای‌گزین می‌شود. |
...
| `SITE_KEYWORD_MAP` | ❌ | — | جفت‌های `keyword:SiteName` جدا‌شده با کاما. دستگاهی که hostname آن شامل کلیدواژه (به‌صورت case-insensitive) باشد به آن سایت اختصاص می‌یابد. مثلاً `dc1:Datacenter1,hq:HQ`. |
| `SCAN_WORKERS` | ❌ | `20` | اندازه thread pool برای اسکن موازی IP. |
| `DEFAULT_SITE_NAME` | ❌ | `Default` | نام سایت پیش‌فرض وقتی هیچ کلیدواژه‌ای تطابق نخورد. |
| `DEFAULT_ROLE_NAME` | ❌ | `Server` | نقش دستگاه NetBox برای سرورها. |
| `DEFAULT_STORAGE_ROLE` | ❌ | `Storage` | نقش دستگاه NetBox برای ذخیره‌سازی. |

> *پیش‌فرض‌های موجود در `sync_all_to_netbox.py` صرفاً CIDR‌های **نمونه/تست** هستند (`192.0.2.0/27` = TEST-NET). بازه‌های واقعی خود را در `.env` تنظیم کنید.
...
اسکریپت نقش‌های inventory item را با **ID ثابت** اختصاص می‌دهد. مطمئن شوید این نقش‌ها در NetBox (`/dcim/inventory-item-roles/`) با همین IDها وجود دارند (به‌ترتیب زیر بسازید، یا ثابت‌های `ROLE_*` را در ابتدای اسکریپت اصلاح کنید):
...
اسکریپت **custom fields** روی دستگاه‌ها می‌نویسد. این‌ها را در NetBox (`/extras/custom-fields/`، نوع شیء `dcim | device`) بسازید:
...
| `redfish_cpu_sockets` | Integer | سوکت‌های CPU |
| `redfish_cpu_cores` | Integer | هسته‌های CPU |
| `redfish_cpu_threads` | Integer | threadهای CPU |
| `redfish_ram_gib` | Integer | RAM (GiB) |
| `redfish_disk_total_gib` | Integer | کل دیسک (GiB) |
...
| `storage_health` | Text | سلامت |
| `storage_firmware` | Text | فریم‌ور |
| `storage_model` | Text | مدل |
| `storage_disk_count` | Integer | تعداد دیسک |
| `storage_total_capacity_gib` | Integer | ظرفیت کل (GiB) |

> حلقه تشخیص آفلاین دستگاه‌ها را با `cf_redfish_enabled=True` / `cf_storage_enabled=True` فیلتر می‌کند (سینتکس فیلتر custom field در NetBox).

### ۳. نقش‌ها و سایت‌های دستگاه

نقش‌های `Server` و `Storage`، سازنده `HPE` و سایت‌ها **به‌طور خودکار** ساخته می‌شوند اگر وجود نداشته باشند. می‌توانید از قبل هم بسازید.
...
اسکریپت:
1. بلافاصله پس از راه‌اندازی یک همگام‌سازی **اولیه** اجرا می‌کند.
2. اجرای روزانه در **۰۰:۰۰** و **۱۲:۰۰** زمان‌بندی می‌کند.
3. هر اقدام را با timestamp و سطح لاگ (`INFO` / `WARN` / `ERROR`) در stdout لاگ می‌کند.
...
برای توقف زمان‌بند `Ctrl+C` بزنید.

### اجرا به‌عنوان سرویس (اختیاری)

برای محیط عملیاتی، زیر systemd، سرویس ویندوز یا کانتینر اجرا کنید تا بعد از ریبوت هم فعال بماند.
...
- شامل fallback مخصوص HPE SmartStorage برای Gen9 iLO4 (که داده کامل Redfish ذخیره‌سازی ندارد)، با تولید pseudo-serial برای HBAهایی که سریال عرضه نمی‌کنند.

**ذخیره‌سازی (XML API مخصوص MSA):**
- HPE MSA 2040، 2042، 2050، 2052، 2060
- تفاوت نام فیلدها بین `show disks` (فریم‌ور جدیدتر) و `show disk-parameters` (فریم‌ور قدیمی‌تر) مدیریت می‌شود.

برای نگاشت کامل نام مدل‌ها به `models.py` مراجعه کنید. مدل‌های خود را آنجا اضافه کنید.

## آیتم‌های انبارداری جمع‌آوری‌شده

**از هر سرور (Redfish):**

| قطعه | منبع | فیلدهای کلیدی |
|----------|--------|------------|
| CPU | `/Systems/1/Processors` | مدل، هسته، thread، سریال |
...
| PSU | `/Chassis/*/Power/PowerSupplies` | مدل، وات، سریال |
...
| قطعه | دستور `show` سِ MSA | فیلدهای کلیدی |
|----------|--------------------|------------|
| دیسک | `show disks` / `show disk-parameters` / `show disk-statistics` | مدل، اندازه، نوع، سریال، سلامت |
...
هر دستگاه کشف‌شده با **سریال‌نامبر** (اصلی) به دستگاه NetBox موجود تطبیق داده می‌شود. اگر سریال نامعتبر/ناموجود باشد، جستجوی ثانویه با **نام + سایت + نقش** انجام می‌شود. این از دستگاه‌های تکراری بین اجراها جلوگیری می‌کند.

برای ذخیره‌سازی، جستجوی ثانویه همچنین از تداخل با سروری که هم‌نام است جلوگیری می‌کند (با بررسی غیبت فیلد سفارشی `bmc_ip`).

## تشخیص آفلاین

پس از هر همگام‌سازی، اسکریپت تمام دستگاه‌هایی که `redfish_enabled=True` (سرورها) یا `storage_enabled=True` (ذخیره‌سازی) دارند را از NetBox می‌خواند. اگر IP ذخیره‌شده BMC/ذخیره‌سازی دستگاه در اسکن فعلی **دیده نشده باشد**، وضعیت آن `status=offline` و فلگ `*_enabled=false` می‌شود. دستگاه **حذف نمی‌شود** — اسکن موفق بعدی آن را دوباره `active` می‌کند.
